# PRÁCTICA NÚMEROS PRIMOS

Se tomaron 4 switches para representar los números decimales del 0 al 15, y por medio de un LED indicar si el número representado es primo no.

Si se representa el número 1101 en binario, 13 en decimal, el LED enciende, pues el número 13 es un número primo.

En la lógica, los números 2, 3, 5, 7, 11 y 13 son determinados como primos, y debe encender el led indicando esa propiedad.

Se deja que el default es el led apagado, para que el led solo encienda si es primo.


<img width="933" height="207" alt="image" src="https://github.com/user-attachments/assets/839cac53-5d86-427e-af8a-942fb57d8724" />



**SIMULACIÓN EN QUESTA SIM**

<img width="981" height="502" alt="image" src="https://github.com/user-attachments/assets/fe591bc8-a9e4-4913-acea-d6a84f4b7ef6" />


**RTL VIEWER**

<img width="918" height="489" alt="image" src="https://github.com/user-attachments/assets/e0b0b2cd-6669-4098-8985-5bd9f079fd0b" />


**DEMOSTRACIÓN FÍSICA**


Número 13: LED ENCENDIDO

<img width="832" height="583" alt="image" src="https://github.com/user-attachments/assets/badabf3b-fbe0-4dc0-8095-2368fa1aa410" />


Número 9: LED APAGADO

<img width="837" height="641" alt="image" src="https://github.com/user-attachments/assets/cc9915ae-e7bf-4ca4-8bd1-79eaaf3abb02" />


Número 0: LED APAGADO

<img width="817" height="633" alt="image" src="https://github.com/user-attachments/assets/10660064-502c-4e76-94c4-dfc6a04911cd" />


Número 5: LED ENCCENDIDO

<img width="821" height="683" alt="image" src="https://github.com/user-attachments/assets/fe77ef19-2465-41e3-9310-67c78930b33a" />
